@testset "Diffusion SMLM" begin
    ...
    # Test that we got the correct return type
    @test isa(result, BasicSMLD)
    ...
    if !isempty(result) && result.camera !== nothing
        ...
        @test img_info.frames_generated > 0
    end
    ...
    @testset "SimInfo for Diffusion" begin
        # Test SimInfo fields specific to diffusion
        result, info = simulate(small_params)
        ...
        @test info.smld_true === nothing  # Diffusion doesn't have ground truth
        @test info.smld_model === nothing  # Diffusion doesn't have model
    end
end
