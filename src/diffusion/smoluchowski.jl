...
- `Tuple{BasicSMLD, SimInfo}`: (smld, info)
    - smld: Single SMLD object containing all emitters across all frames
    - info: SimInfo containing timing and counts
...
function simulate(params::DiffusionSMLMParams;
    ...
    # Track initial emitter count
    n_emitters_initial = length(emitters)
    ...
    info = SimInfo(
        ...
        nothing,  # no smld_true for diffusion
        nothing,  # no smld_model for diffusion
        0,        # n_patterns not applicable for diffusion
        n_emitters_initial,
