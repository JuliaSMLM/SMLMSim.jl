...
- `Tuple{Array{T, 3}, ImageInfo}`: (images, info)
    ...
    - info: ImageInfo containing timing, photon counts, and output dimensions
...
function gen_images(smld::SMLD, psf::AbstractPSF;
    ...
    for (i, frame_num) in enumerate(frames)
        ...
        # Sum photons from emitters in this frame
...
- `Tuple{Matrix{T}, ImageInfo}`: (image, info)
    - image: A 2D camera image as Matrix{T} where T matches the type of emitter.photons
    - info: ImageInfo containing timing, photon counts, and output dimensions
"""
